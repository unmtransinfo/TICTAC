...
drugs <- read_delim("data/attr_drugs.tsv", "\t", skip=1, col_names=c("id", "nct_id", "name"), col_types = "dcc")
...
drugs_leadmine <- read_delim("data/attr_drugs_leadmine.tsv",  "\t", escape_double=F, trim_ws=T)
...
write_delim(drugs_smi, "data/attr_drugs_smi.smi", "\t", col_names=F)
...
#mols <- rcdk::parse.smiles(drugs_smi_sample$smiles)
###
#rcdk::view.table(mols, drugs_smi_sample, cellx=100, celly=60) #not working
###
#molimg <- rcdk::view.molecule.2d(mols)
#h <- 400
#w <- 400
#molimg <- rcdk::view.image.2d(mols[[1]], get.depictor(width=w, height=h))
#plot(c(0, w), c(0, h), asp=1.0, type="n", xlab="", ylab="", pty="m", xaxt="n", yaxt="n")
#rasterImage(molimg, 0, 0, w, h)
#writePNG(molimg, target = "data/molimg.png", text=c(source=R.version.string), metadata=sessionInfo())
#
