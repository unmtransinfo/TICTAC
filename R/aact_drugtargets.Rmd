...
# Identify and enumerate ___drug, disease, target___ combinations associated by study (NCI_ID)
And include references.

## Aggregate disease mentions by study (`NCT_ID`)

```{r}
links <- dner[, .(disease_term := sub(";.*$", "", disease_terms))]
...
```{r}
drug_links <- unique(drugs[!is.na(smiles), .(itv_id = id, nct_id, name, EntityText, smiles)])
...
cid2chembl <- merge(cid2ink, ink2chembl, by.x="InChIKey", by.y="inchikey")
```
