...
message("%8s: %6d", tdl_counts$idgTDL, tdl_counts$N)
plot_ly() %>%
...
```{r}
sddt_links <- unique(merge(sddt_links, mol2tgt, by.x="cid", by.y="CID", allow.cartesian=T))
...
sddt_links <- merge(sddt_links, tgt[, .(uniprot, gene_symbol = sym, idgTDL)], by="uniprot")
sddt_links <- sddt_links[, .(nct_id, drug_name, cid, disease_term, doid, gene_symbol, uniprot, idgTDL)]
knitr::kable(sddt_links[sample(1:nrow(sddt_links), 12)], caption=sprintf("Sample of
