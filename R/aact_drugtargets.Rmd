...
```
```{r}
knitr::kable(merge(studies[, .(N_studies = .N), by="overall_status"], drugs[, .(N_drugs = .N), by="overall_status"], by="overall_status")[order(overall_status)], caption="Drug studies and drugs, by overall_status")
```
...
plot_ly(type="bar", data=merge(studies[, .(N_studies = .N), by="start_year"], drugs[, .(N_drugs = .N), by="start_year"], by="start_year"), x = ~start_year, name="studies", y = ~N_studies) %>%
	...
	layout(title="AACT: Studies and drugs vs. start year", xaxis=list(range=c(1990, 2020), tickangle=-45), yaxis=list(title="N"),
	       barmode="stack",
...
cner_totals[["smi2img"]] <- sprintf("<img src=\"http://pasilla.health.unm.edu/tomcat/cdkdepict/depict/cow/svg?smi=%s&h=50&w=80&zoom=3\">", sapply(cner_totals$smiles, URLencode, reserved=T))
...
```{r message=FALSE}
pubchem <- read_delim("data/aact_drugs_smi_pubchem_cid2ink.tsv", "\t")
...
```{r message=FALSE}
tcrd_tgt <- read_delim("/home/data/TCRD/data/tcrd_targets.tsv", "\t", col_types=cols(.default=col_character()))
setDT(tcrd_tgt)
setnames(tcrd_tgt, old=c("tdl", "fam"), new=c("idgTDL", "idgFamily"))
```
...
atype_counts <- chembl_act[,.(N_molecule = uniqueN(molecule_chembl_id), N_activity = .N), by="assay_type"][order(-N_activity)]
...
tgt <- merge(chembl_tgt, tcrd_tgt, all.x=T, all.y=F, by.x="uniprot", by.y="uniprot")
...
* Also root term "DISEASE" is uninteresting.
...
```{r}
links <- copy(dner)
links[, `:=`(disease_term = sub(";.*$", "", disease_terms), disease_terms = NULL, N_mentions = NULL)]
...
sprintf("study-disease links: %d", nrow(links))
```
...
drug_links <- merge(drug_links, drug2cid, by="itv_id")
...
links <- merge(links, drug_links, by="nct_id")
links <- links[!is.na(doid) & !is.na(cid)]
sprintf("study-drug-disease links: %d", nrow(links))
sprintf("studies with drug-disease links: %d", uniqueN(links$nct_id))
```
...
ink2chembl <- read_delim("data/aact_drugs_ink2chembl.tsv", "\t", col_types = cols(
	.default = col_character(), 
	first_approval=col_integer(), 
	usan_year=col_integer(), 
	withdrawn_year=col_integer()))
setDT(ink2chembl)
...
mol2tgt <- merge(cid2ink, ink2chembl, by.x="InChIKey", by.y="inchikey")
...
### study-drug-disease-target links

```{r}
links <- unique(merge(links, mol2tgt, by.x="cid", by.y="CID", allow.cartesian=T))
sprintf("study-drug-disease-target links: %d", nrow(links))
sprintf("studies: %d ; drugs: %d ; diseases: %d ; targets: %d", uniqueN(links$nct_id), uniqueN(links$cid), uniqueN(links$doid), uniqueN(links$uniprot))
links <- merge(links, tgt[, .(uniprot, gene_symbol = sym, idgTDL)], by="uniprot")
links <- links[, .(nct_id, drug_name, cid, disease_term, doid, gene_symbol, uniprot, idgTDL)]
knitr::kable(links[sample(1:nrow(links), 12)], caption=sprintf("Sample of study-drug-disease-target links (N_total = %d)", nrow(links)))
```

## TDL counts

```{r}
knitr::kable(unique(links[, .(uniprot, idgTDL)])[, .(.N), by="idgTDL"], caption="TDL counts")
```
...
* Study count
* Study phase (completed better)
* Study publications (results better)
* Study date (later better, or at least more novel)

---
_Powered by Rmarkdown._
