...
sprintf("references: %d; NCT_IDs: %d; PMIDs: %d; results_references: %d", nrow(refs), 
        length(unique(refs$nct_id)),
        length(unique(refs$pmid)),
...
sprintf("Drug unique SMILES resolved by LeadMine: %d ; unique intervention IDs: %d", length(unique(cner$smiles)), length(unique(cner$id)))
```
...
cner_totals[["smi2img"]] <- sprintf("<img src=\"http://localhost:8080/cdkdepict/depict/cow/svg?smi=%s&zoom=2.2\">", sapply(cner_totals$smiles, URLencode, reserved=T))
...
sprintf("Mentions by intervention ID: %d / %d (%.1f%%)", nrow(cner_this), length(unique(drugs$id)), 100*nrow(cner_this)/length(unique(drugs$id)))
```
...
sprintf("Mentions by study: %d / %d (%.1f%%)", nrow(cner_this), length(unique(drugs$nct_id)), 100*nrow(cner_this)/length(unique(drugs$nct_id)))
```
...
sprintf("Mentions by drug name: %d / %d (%.1f%%)", nrow(cner_this), length(unique(drugs$name)), 100*nrow(cner_this)/length(unique(drugs$name)))
```
