...
## ChEMBL targets by organism (top 10):
...
* (PubChem) `SMILES` &rarr; `CID`
* (PubChem) `CID` &rarr; `INCHIKEY`
* (ChEMBL) `INCHIKEY` &rarr; `MOLECULE_CHEMBL_ID`
* (ChEMBL) `MOLECULE_CHEMBL_ID` &rarr; `ACTIVITY_ID`
* (ChEMBL) `ACTIVITY_ID` &rarr; `TARGET_CHEMBL_ID`
* (ChEMBL) `TARGET_CHEMBL_ID` &rarr; `COMPONENT_ID`
* (ChEMBL) `COMPONENT_ID` &rarr; `ACCESSION`
* (ChEMBL) `ACTIVITY_ID` &rarr; `DOCUMENT_CHEMBL_ID`
* (ChEMBL) `DOCUMENT_CHEMBL_ID` &rarr; `PUBMED_ID`
