...
`COMPONENT_ID` &rarr;(ChEMBL)&rarr; `ACCESSION`
...
```{r message=FALSE}
chembl_act <- read_delim("data/aact_drugs_chembl_activity_pchembl.tsv", "\t", col_types=cols(
...
```{r message=FALSE}
act2tgt <- read_delim("data/aact_drugs_chembl_act2tgt.tsv", "\t", col_types="cc")
sprintf("ACTIVITY_IDs: %d ; TARGET_CHEMBL_IDs: %d ; pairs: %d",
	uniqueN(act2tgt$activity_id), uniqueN(act2tgt$target_chembl_id), nrow(act2tgt)) 
...
sprintf("ACCESSIONs: %d ; SINGLE_PROTEIN ACCESSIONs: %d",
	uniqueN(tgt2tc$accession), uniqueN(tgt2tc[target_type=="SINGLE PROTEIN", .(accession)]))
```
...
act <- read_delim("data/aact_drugs_chembl_activity_pchembl.tsv", "\t")
sprintf("MOLECULE_CHEMBL_IDs: %d ; TARGET_CHEMBL_IDs: %d ; ACTIVITY_IDs: %d ; DOCUMENT_CHEMBL_IDs: %d",
	uniqueN(act$molecule_chembl_id), uniqueN(act$target_chembl_id),
	uniqueN(act$activity_id), uniqueN(act$document_chembl_id))
```
