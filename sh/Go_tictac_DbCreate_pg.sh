...
N=$(psql -t -d ${DBNAME_AACT} -c "SELECT COUNT(*) FROM ${DBSCHEMA_AACT}.studies")
if [ ! ${N} -gt 0 ]; then
	printf "ERROR: Cannot access existing AACT db (\"${DBNAME_AACT}\")\n"
	exit
else
	printf "Study count (${DBNAME_AACT}): %d\n" "${N}"
fi
#
dropdb --if-exists "${DBNAME}"
psql -c "CREATE DATABASE ${DBNAME} WITH TEMPLATE ${DBNAME_AACT}"
#
psql -d $DBNAME -c "CREATE SCHEMA ${DBSCHEMA}"
#
...
psql -d $DBNAME -c "CREATE TABLE ${DBSCHEMA}.gene_disease_assns ( ${colspecs} )"
#
printf "${header}\n" >${DATADIR}/tmp.csv
cat ${DATADIR}/tictac_genes_disease_associations.csv |sed '1d' >>${DATADIR}/tmp.csv
#
cat ${DATADIR}/tmp.csv \
	|psql -d $DBNAME -c "COPY ${DBSCHEMA}.gene_disease_assns FROM STDIN WITH (FORMAT CSV,DELIMITER ',', HEADER TRUE)"
psql -d $DBNAME -c "COMMENT ON TABLE ${DBSCHEMA}.gene_disease_assns IS 'TICTAC gene-disease associations'"
#
cols_int="\
nstudy \
npub \
...
for col in ${cols_int} ; do
	psql -d $DBNAME -c "ALTER TABLE ${DBSCHEMA}.gene_disease_assns ALTER COLUMN ${col} TYPE INTEGER USING ${col}::INTEGER" 
done
#
cols_float="\
npub_weighted \
...
for col in ${cols_float} ; do
	psql -d $DBNAME -c "ALTER TABLE ${DBSCHEMA}.gene_disease_assns ALTER COLUMN ${col} TYPE FLOAT USING ${col}::FLOAT" 
done
#
###
# Tagger disease NER on AACT study descriptions.
psql -d $DBNAME <<__EOF__
CREATE TABLE ${DBSCHEMA}.descriptions_tagger_disease_matches (
	nct_id VARCHAR(12),
	paragraph INTEGER,
	sentence INTEGER,
	ch_first INTEGER,
...
cat ${DATADIR}/aact_descriptions_tagger_disease_matches.tsv \
	|psql -d $DBNAME -c "COPY ${DBSCHEMA}.descriptions_tagger_disease_matches FROM STDIN WITH (FORMAT CSV,DELIMITER E'\t', HEADER FALSE)"
#
###
# Leadmine chemical NER on AACT study descriptions.
psql -d $DBNAME <<__EOF__
CREATE TABLE ${DBSCHEMA}.descriptions_leadmine_chemical_matches (
	docname VARCHAR(12),
...
cat ${DATADIR}/aact_drugs_leadmine.tsv |sed '1d' \
	|psql -d $DBNAME -c "COPY ${DBSCHEMA}.descriptions_leadmine_chemical_matches FROM STDIN WITH (FORMAT CSV,DELIMITER E'\t', HEADER FALSE)"
#
