...
#############################################################################
def Csv2RowArrays(fin,noheader,delim):
  n_in=0; rows=[];
  csvReader=csv.reader(fin,dialect='excel',delimiter=delim,quotechar='"')
  while True:
    try:
    ...
    rows.append(row[:])
  print >>sys.stderr, 'rows read: %d'%(len(rows))
  return rows

#############################################################################
def Csv2RowDicts(fin,delim):
  rows=[];
  csvReader=csv.DictReader(fin,fieldnames=None,dialect='excel',delimiter=delim)
  while True:
    try:
      row=csvReader.next()
    except:
...
#############################################################################
def CsvExtractColumn2Array(fin,col,numeric,delim,noheader):
  n_in=0; n_data=0;
  vals=[]
  csvReader=csv.reader(fin,dialect='excel',delimiter=delim,quotechar='"')
  while True:
...
#############################################################################
def CsvDedupByCol(fin,fout,col,numeric,delim,noheader):
...
#############################################################################
def CsvSetvalEmptyByCol(fin,fout,col,setval,numeric,delim,noheader):
  n_in=0; n_data=0; n_out=0; n_set=0;
  csvReader=csv.reader(fin,dialect='excel',delimiter=delim,quotechar='"')
  csvWriter=csv.writer(fout,dialect='excel',delimiter=delim,quotechar='"',quoting=csv.QUOTE_NONNUMERIC)
  while True:
    try:
    ...
    ok=False
    if row[col-1]=='':
      row_out[col-1] = int(setval) if numeric else setval
      n_set+=1
    csvWriter.writerow(row_out)
    n_out+=1
  print >>sys.stderr, 'empty cells set to "%s": %d'%(setval,n_set)
  return n_data,n_out
...
Allow both minval and maxval.'''
  ...
  while True:
    ...
    try:
      ...
      print >>sys.stderr, ('ERROR: '+str(e)+' ("'+val+'")')
      val = 0 if numeric else (datetime.datetime.min if chron else '')
      continue

    if type(val) in (types.FloatType,types.IntType,types.LongType):
      if minval!=None and maxval!=None:
        ok = bool(val>=minval and val<=maxval)
      elif minval!=None:
        ok = bool(val>=minval)
      elif maxval!=None:
        ok = bool(val<=maxval)
    elif (type(eqval) in types.StringTypes) and eqval.upper()=='NULL':
      ok = bool(val in (None,''))
    elif eqval!=None:
      ok = bool(val==eqval)
    else:
      ok=False #default is fail
...
operations (one of):
  ...
  --rmemptyrows ............... remove rows with all empty values
...
operations requiring --col[s] or --coltag[s]:
  ...
  --dedupbycol ................ deduplicate rows, based on specified column
  --setval_empty_bycol ........ set empty vals to VAL
...
parameters:
  ...
  --setval VAL ................ val for --setval_empty_bycol
...
'''%{'PROG':PROG}
  ...
  numeric=False;
  chron=False;
  minval=None; maxval=None; eqval=None; setval=None;
  col=0; cols=[];
  coltag=None; coltags=[];
  colB=0; coltagB=None;
  newtag=None;
  ...
  opts,pargs = getopt.getopt(sys.argv[1:],'',['h','v','vv','i=','o=','valfile=',
	...
	'addquotes',
	'fixquotes',
	'list_tags','minval=','maxval=','eqval=','setval=','numeric','chron',
	'addheader=',
	'nmax=','skip=',
	'colpairanalysis',
	'convert2triples',
  ...
  for (opt,val) in opts:
    ...
    elif opt=='--dedupbycol': dedupbycol=True
    elif opt=='--setval_empty_bycol': setval_empty_bycol=True
...
#   if verbose: print >>sys.stderr, '"%s" is column %d.'%(coltag,col)
  ...
  if csv2html:
    ...
    if not col: ErrorExit('ERROR: --col required.')
    n_in,n_out = CsvDedupByCol(fin,fout,col,numeric,delim,noheader)
    print >>sys.stderr, '%s: input file: %s ; rows in: %d'%(PROG,ifile,n_in)
    print >>sys.stderr, '%s: output file: %s ; rows out: %d'%(PROG,fout.name,n_out)

  elif setval_empty_bycol:
    if not col: ErrorExit('ERROR: --col required.')
    n_in, n_out = CsvSetvalEmptyByCol(fin,fout,col,setval,numeric,delim,noheader)
    print >>sys.stderr, '%s: input file: %s ; rows in: %d'%(PROG,ifile,n_in)
    print >>sys.stderr, '%s: output file: %s ; rows out: %d'%(PROG,fout.name,n_out)
