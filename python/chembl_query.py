...
"get_tgt", "get_assay", "getActivityForMol", "getActivityForAssay", "getActivityForTarget", "get_doc"]
  ...
  if args.ifile:
    ...
    ids = chembl_api_utils.Uniprot2ID(args.uniprot, base_url)

  if args.op[:3]=="get" and not (args.ifile or args.ids):
    parser.error('--i or --ids required for operation {0}.'.format(args.op))

  if args.op == "status":
    api_ver,db_ver,status = chembl_api_utils.Status(base_url)
    print('%s, %s, %s'%(api_ver, db_ver, status))

  elif args.op == "list_sources":
    chembl_api_utils.ListSources(args.api_host, args.api_base_path, fout)

  elif args.op == "list_targets":
    chembl_api_utils.ListTargets(args.api_host, args.api_base_path, args.skip, args.nmax, fout)

  elif args.op == "list_mols":
    chembl_api_utils.ListMolecules(args.api_host, args.api_base_path, False, args.skip, args.nmax, fout)

  elif args.op == "list_drugs":
    chembl_api_utils.ListMolecules(args.api_host, args.api_base_path, True, args.skip, args.nmax, fout)

  elif args.op == "list_docs":
    chembl_api_utils.ListDocuments(args.api_host, args.api_base_path, args.skip, args.nmax, fout)

  elif args.op == "list_cells":
    chembl_api_utils.ListCellLines(args.api_host, args.api_base_path, fout)

  elif args.op == "list_tissues":
    chembl_api_utils.ListTissues(args.api_host, args.api_base_path, fout)

  elif args.op == "list_mechanisms":
    chembl_api_utils.ListMechanisms(args.api_host, args.api_base_path, fout)

  elif args.op == "list_assays":
    chembl_api_utils.ListAssays(args.api_host, args.api_base_path, args.skip, args.nmax, fout)

  elif args.op == "get_assay":
    chembl_api_utils.GetAssay(base_url, ids, fout)

  elif args.op == "get_mol":
    chembl_api_utils.GetMolecule(base_url, ids, fout)

  elif args.op == "getActivityForMol":
    chembl_api_utils.GetActivity(ids, 'molecule', args.api_host, args.api_base_path, pmin, fout)

  elif args.op == "getActivityForAssay":
    chembl_api_utils.GetActivity(ids, 'assay', args.api_host, args.pmin, fout)

  elif args.op == "getActivityForTarget":
    chembl_api_utils.GetActivity(ids, 'target', args.api_host, args.pmin, fout)

  elif args.op == "get_tgt":
    chembl_api_utils.GetTarget(ids, base_url, fout)

  elif args.op == "get_doc":
    chembl_api_utils.GetDocument(ids, base_url, fout)
    ...
    chembl_api_utils.SearchAssays(args.assay_source, args.assay_type, args.api_host, args.api_base_path, args.skip, args.nmax, fout)
